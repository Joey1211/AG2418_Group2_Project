    ...
    for epoch in range(num_epochs - start_epoch):
        ...
        for phase in ['train']:
            ...
            for data, data2, data3, data4 in zip(dataloaders['satellite'], dataloaders['street'], dataloaders['drone'],
                ...
                '''
                if use_gpu:
                    inputs = Variable(inputs.to(device).detach())
                    inputs2 = Variable(inputs2.to(device).detach())
                    inputs3 = Variable(inputs3.to(device).detach())
                    labels = Variable(labels.to(device).detach())
                    labels2 = Variable(labels2.to(device).detach())
                    labels3 = Variable(labels3.to(device).detach())

                    if opt.extra_Google:
                        inputs4 = Variable(inputs4.to(device).detach())
                        labels4 = Variable(labels4.to(device).detach())
                else:
